...
import { ISubTask, ISubtasksForm } from '@/app/(main)/tasks/[id]/types'
import { useDeleteSubTask, useUpdateSubTask } from '@/data/hooks/task'
...
export const SubtaskWrapper = ({ taskData, subTask }: ISubtaskWrapper) => {
  ...
  const { mutate: mutateUpdateSubTask } = useUpdateSubTask({ queryClient })
  const { mutate: mutateDeleteSubTask } = useDeleteSubTask({ queryClient })
  const handleOpen = () => {
    setIsOpen((prev) => !prev)
  }
